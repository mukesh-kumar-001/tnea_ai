...
//#region node_modules/.nitro/vite/services/ssr/assets/cutoffs-Y0-BF-fG.js
...
function Cutoffs() {
	...
	const filteredBranches = (0, import_react.useMemo)(() => {
		...
		if (cutoffsResponse?.data) cutoffsResponse.data.forEach((r) => {
			if (r && r.branch_code && !branchMap.has(r.branch_code)) branchMap.set(r.branch_code, {
				code: r.branch_code,
				name: r.branch_name || "Unknown Branch"
			});
		});
	...
	const data = (0, import_react.useMemo)(() => {
		...
		for (const r of filtered) if (r && r.year != null) {
			if (!yearMap.has(r.year)) yearMap.set(r.year, []);
			if (r.cutoff_mark != null) yearMap.get(r.year).push(r.cutoff_mark);
		}
